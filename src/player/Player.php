...
use watermossmc\block\UnknownBlock;
use watermossmc\block\VanillaBlocks;
use watermossmc\command\CommandSender;
use watermossmc\crafting\CraftingGrid;
use watermossmc\data\java\GameModeIdMap;
use watermossmc\entity\animation\Animation;
use watermossmc\entity\animation\ArmSwingAnimation;
use watermossmc\entity\animation\CriticalHitAnimation;
...
use watermossmc\event\inventory\InventoryCloseEvent;
use watermossmc\event\inventory\InventoryOpenEvent;
use watermossmc\event\player\PlayerBedEnterEvent;
use watermossmc\event\player\PlayerBedLeaveEvent;
use watermossmc\event\player\PlayerBlockPickEvent;
use watermossmc\event\player\PlayerChangeSkinEvent;
use watermossmc\event\player\PlayerChatEvent;
use watermossmc\event\player\PlayerDeathEvent;
use watermossmc\event\player\PlayerDisplayNameChangeEvent;
use watermossmc\event\player\PlayerDropItemEvent;
use watermossmc\event\player\PlayerEmoteEvent;
use watermossmc\event\player\PlayerEntityInteractEvent;
use watermossmc\event\player\PlayerEntityPickEvent;
use watermossmc\event\player\PlayerExhaustEvent;
use watermossmc\event\player\PlayerGameModeChangeEvent;
use watermossmc\event\player\PlayerInteractEvent;
use watermossmc\event\player\PlayerItemConsumeEvent;
use watermossmc\event\player\PlayerItemHeldEvent;
use watermossmc\event\player\PlayerItemUseEvent;
use watermossmc\event\player\PlayerJoinEvent;
use watermossmc\event\player\PlayerJumpEvent;
use watermossmc\event\player\PlayerKickEvent;
use watermossmc\event\player\PlayerMissSwingEvent;
use watermossmc\event\player\PlayerMoveEvent;
use watermossmc\event\player\PlayerPostChunkSendEvent;
use watermossmc\event\player\PlayerQuitEvent;
use watermossmc\event\player\PlayerRespawnEvent;
use watermossmc\event\player\PlayerToggleFlightEvent;
use watermossmc\event\player\PlayerToggleGlideEvent;
use watermossmc\event\player\PlayerToggleSneakEvent;
use watermossmc\event\player\PlayerToggleSprintEvent;
use watermossmc\event\player\PlayerToggleSwimEvent;
use watermossmc\event\player\PlayerTransferEvent;
use watermossmc\event\player\PlayerViewDistanceChangeEvent;
use watermossmc\form\Form;
use watermossmc\form\FormValidationException;
...
use watermossmc\inventory\PlayerCraftingInventory;
use watermossmc\inventory\PlayerCursorInventory;
use watermossmc\inventory\TemporaryInventory;
use watermossmc\inventory\transaction\action\DropItemAction;
use watermossmc\inventory\transaction\InventoryTransaction;
use watermossmc\inventory\transaction\TransactionBuilder;
use watermossmc\inventory\transaction\TransactionCancelledException;
use watermossmc\inventory\transaction\TransactionValidationException;
use watermossmc\item\ConsumableItem;
use watermossmc\item\Durable;
use watermossmc\item\enchantment\EnchantmentInstance;
use watermossmc\item\enchantment\MeleeWeaponEnchantment;
use watermossmc\item\Item;
use watermossmc\item\ItemUseResult;
use watermossmc\item\Releasable;
...
use watermossmc\player\chat\StandardChatFormatter;
...
use watermossmc\world\ChunkListener;
use watermossmc\world\ChunkListenerNoOpTrait;
use watermossmc\world\ChunkLoader;
use watermossmc\world\ChunkTicker;
use watermossmc\world\format\Chunk;
use watermossmc\world\Position;
use watermossmc\world\sound\EntityAttackNoDamageSound;
use watermossmc\world\sound\EntityAttackSound;
use watermossmc\world\sound\FireExtinguishSound;
use watermossmc\world\sound\ItemBreakSound;
...
class Player extends Human implements CommandSender, ChunkListener, IPlayer
{
	...
	{
		$ev = new PlayerTransferEvent($this, $address, $port, $message ?? KnownTranslationFactory::watermossmc_disconnect_transfer());
		$ev->call();
		if (!$ev->isCancelled()) {
	...
	public function respawn() : void
	{
		if ($this->server->isHardcore()) {
			if ($this->kick(KnownTranslationFactory::watermossmc_disconnect_ban(KnownTranslationFactory::watermossmc_disconnect_ban_hardcore()))) { //this allows plugins to prevent the ban by cancelling PlayerKickEvent
	...
	protected function actuallyRespawn() : void
	{
		...
		$spawn->getWorld()->requestSafeSpawn($spawn)->onCompletion(
			...
			function () : void {
				if ($this->isConnected()) {
					$this->getNetworkSession()->disconnectWithError(KnownTranslationFactory::watermossmc_disconnect_error_respawn());
				}
			}
		);
	}
