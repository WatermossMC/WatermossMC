...
use watermossmc\block\utils\ChiseledBookshelfSlot;
...
use watermossmc\data\bedrock\block\convert\BlockStateDeserializerHelper as Helper;
use watermossmc\data\bedrock\block\convert\BlockStateReader as Reader;
...
final class BlockStateToObjectDeserializer implements BlockStateDeserializer
{
	...
	private function registerCandleDeserializers() : void
	{
		...
		foreach ([
			...
			$this->map(
				$id,
				fn (Reader $in) => Blocks::CAKE_WITH_DYED_CANDLE()
				->setColor($color)
	...
	private function registerFlatColorBlockDeserializers() : void
	{
		...
		foreach ([
			...
			$this->map(
				$id,
				fn (Reader $in) => Blocks::GLAZED_TERRACOTTA()
				->setColor($color)
	...
	private function registerDeserializers() : void
	{
		...
		$this->map(
			...
			fn (Reader $in) => Blocks::COMPOUND_CREATOR()
			->setFacing(Facing::opposite($in->readLegacyHorizontalFacing()))
		);
		...
		$this->map(
			Ids::ELEMENT_CONSTRUCTOR,
			fn (Reader $in) => Blocks::ELEMENT_CONSTRUCTOR()
			->setFacing(Facing::opposite($in->readLegacyHorizontalFacing()))
		);
		...
		$this->map(
			Ids::MATERIAL_REDUCER,
			fn (Reader $in) => Blocks::MATERIAL_REDUCER()
			->setFacing(Facing::opposite($in->readLegacyHorizontalFacing()))
		);
