...
use watermossmc\errorhandler\ErrorTypeToStringMap;
use watermossmc\network\mcpe\protocol\ProtocolInfo;
use watermossmc\plugin\PluginBase;
use watermossmc\plugin\PluginManager;
use watermossmc\Server;
use watermossmc\thread\ThreadCrashInfoFrame;
...
class CrashDump
{
	...
	private function extraData() : void
	{
		...
		if ($this->server->getConfigGroup()->getPropertyBool(YmlServerProperties::AUTO_REPORT_SEND_SETTINGS, true)) {
			...
			if (($watermossmcDotYml = @file_get_contents(Path::join($this->server->getDataPath(), "watermossmc.yml"))) !== false) {
				$this->data->watermossmcDotYml = $watermossmcDotYml;
			}
		}
		$extensions = [];
