...
final class PacketHandler
{
            ...
            switch ($pid) {
                ...
                case ProtocolInfo::LOGIN_PACKET:
                    ...
                    try {
                        ...
                        $jwt = self::buildServerHandshakeJwt($serverPublicB64, $keys['private'], $serverSalt);
    ...
    private static function buildServerHandshakeJwt(string $pubBase64, string $priv, string $salt): string
    {
        ...
        $header = json_encode(["alg" => "ES384", "x5u" => $pubBase64], \JSON_UNESCAPED_SLASHES);
        if ($header === false) {
            throw new RuntimeException("Failed to encode header");
        }
        $payload = json_encode(["salt" => base64_encode($salt)], \JSON_UNESCAPED_SLASHES);
        if ($payload === false) {
