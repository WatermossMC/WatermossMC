...
use watermossmc\event\player\PlayerPreLoginEvent;
...
class LoginPacketHandler extends PacketHandler
{
	...
	public function handleLogin(LoginPacket $packet) : bool
	{
		...
		if (!$this->server->isWhitelisted($playerInfo->getUsername())) {
			$ev->setKickFlag(PlayerPreLoginEvent::KICK_FLAG_SERVER_WHITELISTED, KnownTranslationFactory::watermossmc_disconnect_whitelisted());
		}

		$banMessage = null;
		if (($banEntry = $this->server->getNameBans()->getEntry($playerInfo->getUsername())) !== null) {
			$banReason = $banEntry->getReason();
			$banMessage = $banReason === "" ? KnownTranslationFactory::watermossmc_disconnect_ban_noReason() : KnownTranslationFactory::watermossmc_disconnect_ban($banReason);
		} elseif (($banEntry = $this->server->getIPBans()->getEntry($this->session->getIp())) !== null) {
			$banReason = $banEntry->getReason();
			$banMessage = KnownTranslationFactory::watermossmc_disconnect_ban($banReason !== "" ? $banReason : KnownTranslationFactory::watermossmc_disconnect_ban_ip());
		}
		if ($banMessage !== null) {
