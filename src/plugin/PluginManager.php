...
use watermossmc\event\EventPriority;
use watermossmc\event\HandlerListManager;
use watermossmc\event\Listener;
use watermossmc\event\ListenerMethodTags;
use watermossmc\event\plugin\PluginDisableEvent;
use watermossmc\event\plugin\PluginEnableEvent;
use watermossmc\event\RegisteredListener;
...
use watermossmc\permission\PermissionParser;
...
class PluginManager
{
	...
	private function internalLoadPlugin(string $path, PluginLoader $loader, PluginDescription $description) : ?Plugin
	{
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_load($description->getFullName())));

		$dataFolder = $this->getDataDirectory($path, $description->getName());
		if (file_exists($dataFolder) && !is_dir($dataFolder)) {
			$this->server->getLogger()->critical($language->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
				$description->getName(),
				KnownTranslationFactory::watermossmc_plugin_badDataFolder($dataFolder)
			)));
			return null;
		}
		...
		if (!class_exists($mainClass, true)) {
			$this->server->getLogger()->critical($language->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
				$description->getName(),
				KnownTranslationFactory::watermossmc_plugin_mainClassNotFound()
			)));
			return null;
		}
		if (!is_a($mainClass, Plugin::class, true)) {
			$this->server->getLogger()->critical($language->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
				$description->getName(),
				KnownTranslationFactory::watermossmc_plugin_mainClassWrongType(Plugin::class)
			)));
			return null;
		}
		...
		if (!$reflect->isInstantiable()) {
			$this->server->getLogger()->critical($language->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
				$description->getName(),
				KnownTranslationFactory::watermossmc_plugin_mainClassAbstract()
			)));
			return null;
		}
		...
		foreach ($description->getPermissions() as $permsGroup) {
			foreach ($permsGroup as $perm) {
				if ($permManager->getPermission($perm->getName()) !== null) {
					$this->server->getLogger()->critical($language->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
						$description->getName(),
						KnownTranslationFactory::watermossmc_plugin_duplicatePermissionError($perm->getName())
					)));
					return null;
				}
			}
		...
		foreach ($loaders as $loader) {
			foreach ($files as $file) {
				...
				try {
					...
					$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
						$file,
						KnownTranslationFactory::watermossmc_plugin_invalidManifest($e->getMessage())
					)));
					$loadErrorCount++;
					continue;
				} catch (\RuntimeException $e) { //TODO: more specific exception handling
					$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError($file, $e->getMessage())));
				...
				if ($this->graylist !== null && !$this->graylist->isAllowed($name)) {
					$this->server->getLogger()->notice($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
						$name,
						$this->graylist->isWhitelist() ? KnownTranslationFactory::watermossmc_plugin_disallowedByWhitelist() : KnownTranslationFactory::watermossmc_plugin_disallowedByBlacklist()
					)));
				...
				if (($loadabilityError = $loadabilityChecker->check($description)) !== null) {
					$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError($name, $loadabilityError)));
					$loadErrorCount++;
					continue;
				}

				if (isset($triage->plugins[$name]) || $this->getPlugin($name) instanceof Plugin) {
					$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_duplicateError($name)));
					$loadErrorCount++;
					continue;
				}

				if (str_contains($name, " ")) {
					$this->server->getLogger()->warning($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_spacesDiscouraged($name)));
				}
	...
	public function loadPlugins(string $path, int &$loadErrorCount = 0) : array
	{
		...
		while (count($triage->plugins) > 0) {
			...
			if ($loadedThisLoop === 0) {
				...
				foreach (Utils::stringifyKeys($triage->plugins) as $name => $file) {
					if (isset($triage->dependencies[$name])) {
						...
						if (count($unknownDependencies) > 0) {
							$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError(
								$name,
								KnownTranslationFactory::watermossmc_plugin_unknownDependency(implode(", ", $unknownDependencies))
							)));
							unset($triage->plugins[$name]);
							$loadErrorCount++;
						}
				...
				foreach (Utils::stringifyKeys($triage->plugins) as $name => $file) {
					$this->server->getLogger()->critical($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_loadError($name, KnownTranslationFactory::watermossmc_plugin_circularDependency())));
					$loadErrorCount++;
				}
				break;
			}
	...
	{
		if (!$plugin->isEnabled()) {
			$this->server->getLogger()->info($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_enable($plugin->getDescription()->getFullName())));
			...
			if ($plugin->isEnabled()) { //the plugin may have disabled itself during onEnable()
				...
				$this->server->getLogger()->critical($this->server->getLanguage()->translate(
					KnownTranslationFactory::watermossmc_plugin_enableError(
						$plugin->getName(),
						KnownTranslationFactory::watermossmc_plugin_suicide()
					)
				));
	...
	{
		if ($plugin->isEnabled()) {
			$this->server->getLogger()->info($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_plugin_disable($plugin->getDescription()->getFullName())));
