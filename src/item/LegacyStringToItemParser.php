...
use watermossmc\data\bedrock\item\ItemTypeDeserializeException;
 ...
 *
 * WARNING: This class does NOT support items added during or after WatermossMC 5.0.0. Use StringToItemParser for
 * modern items.
 */
final class LegacyStringToItemParser
{
	...
	private static function make() : self
	{
		...
		$mappingsRaw = Filesystem::fileGetContents(Path::join(\watermossmc\RESOURCE_PATH, 'item_from_string_bc_map.json'));

		$mappings = json_decode($mappingsRaw, true);
		if (!is_array($mappings)) {
			throw new AssumptionFailedError("Invalid mappings format, expected array");
		}

		foreach (Utils::promoteKeys($mappings) as $name => $id) {
			if (!is_string($id)) {
				throw new AssumptionFailedError("Invalid mappings format, expected string values");
			}
