...
use watermossmc\scheduler\BulkCurlTask;
use watermossmc\scheduler\BulkCurlTaskOperation;
use watermossmc\timings\TimingsHandler;
use watermossmc\utils\AssumptionFailedError;
use watermossmc\utils\InternetException;
use watermossmc\utils\InternetRequestResult;
...
class TimingsCommand extends VanillaCommand
{
	public function __construct()
	{
		parent::__construct(
			"timings",
			KnownTranslationFactory::watermossmc_command_timings_description(),
			KnownTranslationFactory::watermossmc_command_timings_usage()
		);
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args)
	{
		...
		if ($mode === "on") {
			if (TimingsHandler::isEnabled()) {
				$sender->sendMessage(KnownTranslationFactory::watermossmc_command_timings_alreadyEnabled());
				return true;
			}
			TimingsHandler::setEnabled();
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_enable());

			return true;
		} elseif ($mode === "off") {
			TimingsHandler::setEnabled(false);
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_disable());
			return true;
		}

		if (!TimingsHandler::isEnabled()) {
			$sender->sendMessage(KnownTranslationFactory::watermossmc_command_timings_timingsDisabled());
		...
		if ($mode === "reset") {
			...
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_reset());
		} elseif ($mode === "merged" || $mode === "report" || $paste) {
			$timingsPromise = TimingsHandler::requestPrintTimings();
			Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_collect());
	...
	private function createReportFile(array $lines, CommandSender $sender) : void
	{
		...
		Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_timingsWrite($timings));
	}
		...
		$sender->getServer()->getAsyncPool()->submitTask(new BulkCurlTask(
			...
			function (array $results) use ($sender, $host) : void {
				...
				if (is_array($response) && isset($response["id"])) {
					Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_timingsRead(
					...
					Command::broadcastCommandMessage($sender, KnownTranslationFactory::watermossmc_command_timings_pasteError());
				}
			}
		));
	}
