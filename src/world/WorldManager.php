...
use watermossmc\event\world\WorldInitEvent;
use watermossmc\event\world\WorldLoadEvent;
use watermossmc\event\world\WorldUnloadEvent;
use watermossmc\lang\KnownTranslationFactory;
use watermossmc\player\ChunkSelector;
...
class WorldManager
{
	...
	public function unloadWorld(World $world, bool $forceUnload = false) : bool
	{
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_unloading($world->getDisplayName())));
	...
	public function loadWorld(string $name, bool $autoUpgrade = false) : bool
	{
		...
		if (count($providers) !== 1) {
			$this->server->getLogger()->error($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_loadError(
				$name,
				count($providers) === 0 ?
					KnownTranslationFactory::watermossmc_level_unknownFormat() :
					KnownTranslationFactory::watermossmc_level_ambiguousFormat(implode(", ", array_keys($providers)))
			)));
			return false;
		}
		...
		try {
			...
			$this->server->getLogger()->error($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_loadError(
				$name,
				KnownTranslationFactory::watermossmc_level_corrupted($e->getMessage())
			)));
			return false;
		} catch (UnsupportedWorldFormatException $e) {
			$this->server->getLogger()->error($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_loadError(
				$name,
				KnownTranslationFactory::watermossmc_level_unsupportedFormat($e->getMessage())
			)));
			return false;
		}
		...
		if ($generatorEntry === null) {
			$this->server->getLogger()->error($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_loadError(
				$name,
				KnownTranslationFactory::watermossmc_level_unknownGenerator($provider->getWorldData()->getGenerator())
			)));
			return false;
		}
		try {
			...
			$this->server->getLogger()->error($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_loadError(
				$name,
				KnownTranslationFactory::watermossmc_level_invalidGeneratorOptions(
		...
		if (!($provider instanceof WritableWorldProvider)) {
			...
			$this->server->getLogger()->notice($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_conversion_start($name)));
			...
			$this->server->getLogger()->notice($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_conversion_finish($name, $converter->getBackupPath())));
		}
	...
	public function generateWorld(string $name, WorldCreationOptions $options, bool $backgroundGeneration = true) : bool
	{
		...
		if ($backgroundGeneration) {
			$this->server->getLogger()->notice($this->server->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_backgroundGeneration($name)));
				...
				$world->orderChunkPopulation($chunkX, $chunkZ, null)->onCompletion(
					static function () use ($world, &$done, $total) : void {
						...
						if (intdiv($oldProgress, 10) !== intdiv($newProgress, 10) || $done === $total || $done === 1) {
							$world->getLogger()->info($world->getServer()->getLanguage()->translate(KnownTranslationFactory::watermossmc_level_spawnTerrainGenerationProgress(strval($done), strval($total), strval($newProgress))));
						}
					},
