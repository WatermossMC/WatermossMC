...
namespace watermossmc\server_phar_stub;
...
function preparePharCacheDirectory() : string
{
	...
	$i = 0;
	do {
		$tmpPath = sys_get_temp_dir() . '/WatermossMC-phar-cache.' . $i;
		$i++;
...
/**
 * Prepares a decompressed .tar of WatermossMC.phar in the system temp directory for loading code from.
 *
...
echo "Preparing WatermossMC.phar decompressed cache...\n";
$start = hrtime(true);
$cacheName = preparePharCache($tmpDir, __FILE__);
echo "Cache ready at $cacheName in " . number_format((hrtime(true) - $start) / 1e9, 2) . "s\n";

define('watermossmc\ORIGINAL_PHAR_PATH', __FILE__);
require 'phar://' . str_replace(DIRECTORY_SEPARATOR, '/', $cacheName) . '/src/Watermoss.php';
